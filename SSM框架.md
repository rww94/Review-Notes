...
- [10 springMVC架构流程](#10-springmvc架构流程)
...
# 10 springMVC架构流程
&emsp;&emsp;springMVC包括前端控制器，处理器映射器，处理器适配器，处理器，视图解析器和视图。
&emsp;&emsp;当用户发送请求时，先被前端控制器锁拦截，前端控制器收到请求后调用处理器映射器返回处理器执行链（包含相应的处理器和处理器拦截器）。前端控制器获取到处理器执行链后，调用处理器适配器获取到相应的处理器，处理器处理完数据后  ，返回相应的逻辑视图给处理器适配器，在通过处理器适配器返回给前端控制器，然后交给视图解析器进行视图渲染。