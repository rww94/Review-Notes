...
- [4 类加载机制](#4-类加载机制)
    ...
    - [4.3 类加载器](#43-类加载器)
        - [4.3.1 Class文件的来源](#431-class文件的来源)
        - [4.3.2 加载Class文件的加载器](#432-加载class文件的加载器)
...
## 4.3 类加载器
### 4.3.1 Class文件的来源
* 1、Java内部自带核心类，位于$JAVA_HOME/jre/lib$
* 2、Java的扩展类，位于$JAVA_HOME/jre/lib/ext$目录下
* 3、自己开发的项目中的第三方jar包，位于我们项目的目录下，比如WEB-INF/lib目录
### 4.3.2 加载Class文件的加载器
* 1、Java核心类由一个名叫BootstrapClassLoader加载器负责加载，这个类加载器被称为“根加载器或者引导加载器”，BootstrapClassLoader不继承ClassLoader，是有JVM内部实现。
* 2、Java拓展类，是有ExtClassLoader负责加载，被称为“拓展类加载器”。
* 3、项目中编写的类，是有AppClassLoader负责加载，被称为“系统类加载器”。
